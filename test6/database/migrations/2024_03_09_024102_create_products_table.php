...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('products', function (Blueprint $table) {
            $table->increments('id');
            $table->string('name', 255)->comment('Nombre descriptivo del producto');
            $table->string('description', 255)->nullable()->comment('Descripción del producto');
            $table->float('base_cost',8,4)->comment("Costo de producción del producto");
            $table->float('base_price',8,4)->comment("Precio de venta del producto");
            $table->integer('category_id')->unsigned()->comment('Identificador de la categoría del producto');
